...
public class EssentialsUpgrade {
        ...
        for (File file : userFiles) {
            ...
            try {
                ...
                if (config.hasProperty("home") && !config.hasProperty("home.default")) {
                    @SuppressWarnings("unchecked") final List<Object> vals = (List<Object>) config.getProperty("home");
                    if (vals == null) {
                        continue;
                    }
        ...
        for (File file : userFiles) {
            ...
            try {
                ...
                if (config.hasProperty("powertools")) {
                    @SuppressWarnings("unchecked") final Map<Integer, Object> powertools = (Map<Integer, Object>) config.getProperty("powertools");
                    if (powertools == null) {
                        continue;
                    }
    ...
    private void moveUsersDataToUserdataFolder() {
        ...
        for (String username : usersConfig.getKeys(null)) {
            ...
            if (!user.hasHome()) {
                @SuppressWarnings("unchecked") final List<Object> vals = (List<Object>) usersConfig.getProperty(username + ".home");
                if (vals != null) {
                    World world = ess.getServer().getWorlds().get(0);
                    if (vals.size() > 5) {
